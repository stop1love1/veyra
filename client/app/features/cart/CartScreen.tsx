...
export function CartScreen({ g, embed }: ScreenProps) {
  ...
  const body = lines.length === 0 ? (
    <div className="v-empty v-empty-cart">
      <div className="v-empty-ic v-empty-ic-lg"><Ic name="bag" size={40} /></div>
      <div className="v-empty-title">{g.t('emptyCartTitle')}</div>
      <div className="v-muted">{g.t('emptyCartBody')}</div>
      <Btn variant="primary" size="lg" icon="map" onClick={() => (embed ? g.closeWorldPanel() : g.go('world'))}>{g.t('emptyCartCta')}</Btn>
    </div>
  ) : (
    <React.Fragment>
      <div className="v-cart-list">
        {lines.map((ln, i) => {
          // Guard against stale/removed product ids in a persisted cart: a
          // missing product would otherwise crash the whole cart render.
          ...
          return (
            <div key={i} className="v-cart-item">
              ...
              <div className="v-cart-right">
                <button className="v-cart-del" onClick={() => g.removeItem(i)} aria-label={g.t('aDelItem') + ' · ' + VEYRA.tx(p.name, g.lang)}><Ic name="close" size={16} /></button>
